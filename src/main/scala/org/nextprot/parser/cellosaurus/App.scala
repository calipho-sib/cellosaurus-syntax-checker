...
object App {
  ...
  codec.onMalformedInput(CodingErrorAction.IGNORE) // onMalformedInput(CodingErrorAction.REPLACE)
  ...
  def main(args: Array[String]) {
    ...
    for (line <- Source.fromFile(args(0)).getLines()) {
      if (line.contains("�")) { println(line); nonUTF8cnt += 1 }
    ...
    Entries.foreach(entry => {
      ...
      if (!(entry(0).startsWith("ID   ") && entry(1).startsWith("AC   "))) { Console.err.println("Severe error: Missing ID/AC line at " + entry(0) + " Please correct before re-check"); exit }
      entry.foreach(entryline => { //println(entryline)
        ...
        if (entryline.startsWith("ID   ")) { id = entrylinedata; idlist += id }
          ...
          if (oxlist(1) == " ! Homo sapiens" && (taxid != "9606")) Console.err.println("Wrong taxid at: " + entryline)
    ...
    println(nonUTF8cnt + " non-UTF8 character containing lines")
  }
}
